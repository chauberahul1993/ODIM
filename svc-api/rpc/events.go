...
// GetEventSubscriptionsCapabilities defines the RPC call function for
// the DoGetEventSubscriptionCapabilities from events micro service
func DoGetEventSubscriptionsCapabilities(req eventsproto.EventRequest) (*eventsproto.EventSubResponse, error) {
	...
	resp, err := events.GetEventSubscriptionsCapabilities(context.TODO(), &req)
	if err != nil {
		return nil, fmt.Errorf("error: RPC error: %v", err)
	}
	defer conn.Close()
